...
class Vector[A](override val c: A*) extends Point[A](c:_*) {
  ...
  def cos(v: Vector[A])(implicit f: A => Num[A]) = (this ** v)/(this.mod*v.mod)
  //def x(v: Vector[A])(implicit f: A => Num[A]) = (c zip v.c).map(x => x._1 * x._2).reduceLeft(_ + _) // vector product
}
